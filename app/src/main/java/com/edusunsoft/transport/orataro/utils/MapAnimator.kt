...
class MapAnimator private constructor() {
    ...
    fun animateRoute(googleMap: GoogleMap, bangaloreRoute: List<LatLng>) {
        ...
        val optionsBackground = PolylineOptions().add(bangaloreRoute[0]).color(GREY).width(10f)
        backgroundPolyline = googleMap.addPolyline(optionsBackground)

        optionsForeground = PolylineOptions().add(bangaloreRoute[0]).color(Color.BLACK).width(10f)
